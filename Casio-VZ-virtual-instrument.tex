...
\begin{equation}
  \label{eqn:f2p}
  p = 69 + 12\,\log_2\!\left(\dfrac{f_0}{440\,\text{Hz}}\right)
  \qquad\text{\TT{p=f2p(f)}}
\end{equation}
measured in semitones. The value of $440\,\text{Hz}$ in the denominator is the
concert pitch. The offset of $69$ makes the pitch computed by \cref{eqn:f2p}
compatible with a MIDI frequency data value. According to the MIDI
\CHECK{tuning} standard \TODO{\cite{}}, $p$ takes values from the interval
$0.00\leq p\leq 127.99$ with the integer part corresponding to a note number and
the decimal fraction, with two digits precision, to a detune. MIDI note number
$p=0$ stands for C$_{-1}$, $p=69$ for the concert pitch A$_4$, and $p=127$
stands for B$_9$.

Solving \cref{eqn:f2p} for $f_0$, we obtain a formula for converting pitch to
frequency:
\begin{equation}
  \label{eqn:p2f}
  f_0 = 2^{\frac{p-69}{12}}\cdot 440\,\text{Hz}
  \qquad\text{\TT{f=p2f(p)}}.
\end{equation}
...
\begin{figure*}[h!]
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    ...
    \begin{tikzpicture}[x=2.5cm, y=2.5cm]
      ...
      \draw[thin,->] (0,-0.2) -- (0,1.2) node[anchor=east] {$\varphi_{\text{PD}}(\varphi)$};
      \node[anchor=north east] at (0,0) {$0$};
      %
...
\begin{figure*}
  ...
  \caption{Example of phase distortion modulation of the function
    $\sin\!\big(\varphi-\frac{\pi}{2}\big)$ adapted from \cite[p.\,20--21,
    Figs.\,1--3]{CZ1manual}\label{fig:PDMCzExample}. Phase angles $\varphi<0$
    and $\varphi\geq 2\pi$ are wrapped into the base interval
    $0\leq\varphi<2\pi.$}
\end{figure*}
Six out of eight VZ-1 wavetable samples are phase-shifted and phase-distorted 
sine waves:
\begin{equation}
  x(\varphi) = \sin\!\left(\varphi_{\text{PD}}(\varphi)-\frac{\pi}{2}\right)\!.
\end{equation} 
\cref{fig:PDMCzExample} shows some examples.

\ISSUE{Actually, VZ-1 seems to produce the inverted waveforms:
\begin{equation}
  x_{\text{VZ-1}}(\varphi) 
...
\REMARK{An alternate phase distortion characteristic $f(x)$ with domain $0\leq
x < 1$ taking values $0\leq f(x) < 1$ may be needed for Reaktor
implementation:
\begin{equation}
  f(x) = 
  \begin{cases}
